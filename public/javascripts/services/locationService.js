define(function(require) {
  ...
  return [function() {
    ...
    this.typeFilterValidates = function(flags) {
      return _.isEmpty(this.lastSearch.filterProp) || _.reduce(this.lastSearch.filterProp, function(result, value, key) {
        return result && (flags[value] || value==="open");
      }, true);
    };

    this.distanceFilterValidates = function(distance) {
      return this.range >= distance;
    };

    this.openTimeFilterValidates = function(openTimes) {
      return (!this.lastSearch.filterByOpenNow && !_.includes(this.lastSearch.filterProp, 'open')) || this.isOpen(openTimes);
    };

    this.filterValidates = function(location) {
      var typeFilterValidates = this.typeFilterValidates(location.flags);
      var distanceFilterValidates = this.distanceFilterValidates(location.distance);
      var openTimeFilterValidates =this.openTimeFilterValidates(location.openTimes);
      return typeFilterValidates && distanceFilterValidates && openTimeFilterValidates;
    };

    // TODO : move to time util service
    this.isOpen = function(openTimes) {
